...
ivml.visualElements.Bars = function Bars(chartController, barController, $timeout) {
    ...
    var Bars = {
        ...
        events: {
            ...
            dragstartE: null,    //@e dragstart event for bars
      	    dragE: null,	  //@e drag event for bars
            dragendE: null 	 //@e dragend event for bars
	}
    ...
    // Bars or BarElement? Which version of IVML should I be updating...
   // if (Bars.events.dragstartE || Bars.events.dragE || Bars.events.dragendE) {
...
ivml.visualElements.Plot = function (element, controller, $timeout) {
    var Plot = {
        ...
        events: {
            ...
            xbrushstart: null,//@e function that will be called when the horizontal brush starts.  Will pass the d3.svg.brush element of the plot as the first parametter.  Setting the function disables xbrushstart, xbrush, xbrushend, ybrushstart, ybrush, ybrushend.
            brush: null,//@e function that will be called when the two dimensional brush is brushed.  Will pass the d3.svg.brush element of the plot as the first parameter.  Setting the function disables xbrushstart, xbrush, xbrushend, ybrushstart, ybrush, ybrushend.
            brushend: null,//@e function that will be called when the two dimensional brush ends.  Will pass the d3.svg.brush element of the plot as the first parameter.  Setting the function disables xbrushstart, xbrush, xbrushend, ybrushstart, ybrush, ybrushend.
...
ivml.visualElements.Rectangles = function Rectangles(controller, $timeout) {
    var Rects = {
        ...
        events: {
            ...
            mouseOutE: null, //@e mouse out event
            clickE: null,    //@e mouse click event
	        dragstartE: null, //@e dragstart event
            dragE: null,	//@e drag event rects
      	    dragendE: null	//@e dragend event
        }
    }
    ...
    Rects.redraw = function () {
        ...
        if (Rects.events.dragE) {
            ...
            function draginit(d){
                d3.event.sourceEvent.stopPropagation();
                // replace fill color with variable for user input function
                Rects.events.dragE(d, i, d3.select(this).attr('style', 'fill: yellow; stroke: black; stroke-width: 3px;'))
            }

            function dragmove(d){
                var svgwidth = Rects.elements.controller.plotObject.attributes.width; //margin of plotObject
                var svginterval = svgwidth/(Rects.elements.controller.plotObject.attributes.xmax-Rects.elements.controller.plotObject.attributes.xmin); //number of pixels between whole numbers in coordinate plot
                Rects.events.dragE(d, i, d3.select(this)
                    .attr("x", function(d, i){
                        if (d3.event.x < 0) {return 0}
                        //else if (d3.event.x + returnw(d,i) > svgwidth){ return (svgwidth - returnw(d,i))}
                        else {return d3.event.x}}));//xs(Math.floor(d3.event.x/svginterval)) } })); //this function ensures value will be a whole number         
                //console.log("Start value changed to: ", Math.floor(d3.event.x/svginterval));
            }

            function dragfin(d,){
                Rects.events.dragE(d, i, d3.select(this).attr('style', 'stroke: black; stroke-width: 0px;')
                .attr('fill', Rects.attributes.fill))
            ...
            rects.attr("cursor", "ew-resize")
            .on('dragstart', function(d, i){
                Rects.events.dragE(d, i, d3.select(this).call(drag))
            })
            .on('drag', function (d, i) {
                window.dragThis = this;
                Rects.events.dragE(d, i, d3.select(this).call(drag)) 
